...
// =================================================================================================
// TEMPORARY ADMIN FUNCTION - REMOVE AFTER USE
// =================================================================================================
/**
 * TEMPORARY ADMIN FUNCTION
 * * A temporary HTTP function to grant a user admin privileges.
 * TRIGGER URL: /makeAdmin?uid=<USER_UID>&key=<SECRET_KEY>
 * * SECURITY: After use, this function should be removed and redeployed.
 */
exports.makeAdmin = functions.https.onRequest(async (req, res) => {
  // A simple secret key to prevent unauthorized access.
  // CHANGE THIS to a random, hard-to-guess string.
  const SECRET_KEY = "Yb07tw44!";

  if (req.query.key !== SECRET_KEY) {
    console.log("Unauthorized attempt to access makeAdmin function.");
    return res.status(401).send("Unauthorized: Invalid secret key.");
  }

  const uid = req.query.uid;
  if (!uid) {
    return res.status(400).send("Bad Request: Please provide a 'uid' query parameter.");
  }

  try {
    // Note: The custom claim MUST be { isAdmin: true } to match the front-end code.
    // The existing setUserAdminClaim function uses { admin: true }, which will not work
    // with the auth.js logic we have implemented.
    await admin.auth().setCustomUserClaims(uid, { isAdmin: true });
    
    // Also update the Firestore document for consistency, although the primary check is the token claim.
    await db.collection('users').doc(uid).update({ isAdmin: true });

    console.log(`Success! Admin claim set for user: ${uid}`);
    return res.status(200).send(`Success! Admin claim has been set for user ${uid}. Please log out and log back in to see the changes.`);
  } catch (error) {
    console.error("Error setting custom claim:", error);
