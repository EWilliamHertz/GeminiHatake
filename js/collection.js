/**
 * HatakeSocial - My Collection Page Script (v14 - Booster Pack Simulator)
 *
 * This script handles all logic for the my_collection.html page.
 * - NEW: Adds a "Booster Pack Simulator" feature using MTGJSON data.
 * - NEW: Fetches a comprehensive list of sets to populate the simulator dropdown.
...
    // =====================================================================
    // NEW: BOOSTER PACK SIMULATOR LOGIC
    // =====================================================================
...
            if (!response.ok) throw new Error('Could not fetch set list.');
...
                .filter(set => ['core', 'expansion', 'masters', 'draft_innovation'].includes(set.type) && set.isOnlineOnly === false)
...
    // Main function to generate the booster
    generateBtn.addEventListener('click', async () => {
        const setCode = setSelect.value;
        statusEl.textContent = `Fetching data for ${setCode}...`;
        resultsContainer.innerHTML = '<div class="col-span-full text-center p-4"><i class="fas fa-spinner fa-spin text-4xl text-blue-500"></i></div>';
        
        try {
...
                // Using Scryfall for images as it's a reliable image provider
...
    // --- Initial Load ---
    loadCardList('collection');
    populateSetList(); // Populate the booster simulator dropdown on page load
