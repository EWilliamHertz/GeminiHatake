/**
 * HatakeSocial - Community Page Script
 * Merges logic for Friends and Groups into a single, tabbed interface.
 */
document.addEventListener('authReady', (e) => {
    ...
    mainTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            mainTabContents.forEach(content => content.id === targetContentId ? content.classList.remove('hidden') : content.classList.add('hidden'));
        });
    });

    // --- FRIENDS LOGIC ---
    const friendsSubTabs = document.querySelectorAll('.friend-tab-button');
    const friendsTabContents = document.querySelectorAll('.friend-sub-tab-content');
...
document.addEventListener('authReady', (e) => {
    ...
    // ... all other group-related DOM elements
...
document.addEventListener('authReady', (e) => {
    ...
    // --- Initial Load for Community Page ---
    if (currentUser) {
        // Initial load for Friends Tab (default active tab)
        loadFriendsList();
        loadFriendRequests();

        // Initial load for Groups Tab (loads in background)
        loadMyGroups();
        loadDiscoverGroups();
