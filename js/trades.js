/**
 * HatakeSocial - Advanced Trades Page Script (v20.1 - Complete Code)
 *
 * This script provides a comprehensive and secure trading system.
 * - FIX: Restored all previous UI functionality, including working tabs, trade proposal modal,
 * counter-offers, binder view, and trade analysis. This file is now complete with no placeholders.
 * - MERGED: Correctly integrated the Stripe Connect escrow payment system into the existing trade flow.
 * - NEW: Trades involving money now trigger a secure payment modal upon acceptance.
 * - NEW: A "Become a Seller" section appears for users who haven't set up Stripe Connect.
 * - NEW: The "Confirm Receipt" action now securely triggers the Cloud Function to capture and release funds.
 */
document.addEventListener('authReady', (e) => {
    ...
    const handleTradeAction = async (action, tradeId) => {
        ...
        if (action === 'accepted') {
            ...
            // If the current user is the one who needs to pay, open the payment modal immediately
