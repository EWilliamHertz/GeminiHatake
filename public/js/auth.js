/**
* HatakeSocial - Merged Authentication & Global UI Script (v25 - FINAL FIX)
...
document.addEventListener('DOMContentLoaded', async () => {
    // ** FIX: Dynamically import the currency module **
    const { initCurrency } = await import('./modules/currency.js');
    ...
    // This is part of your original, working code. It is being restored.
    window.HatakeSocial = {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        // Initialize currency first based on user status
        if (user) {
            ...
            if (userDoc.exists) {
                ...
                await initCurrency(); // New user, default
            }
        } else {
            await initCurrency(); // Logged out, default
        }
        
        // Dispatch the ready event now that auth and currency are set
        ...
        if (user) {
            ...
            let unsubscribeUserDoc = userDocRef.onSnapshot(async (doc) => {
                if (doc.exists) {
                    if (unsubscribeUserDoc) unsubscribeUserDoc(); // Unsubscribe after first read
