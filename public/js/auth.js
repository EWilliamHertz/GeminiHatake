...
if (user) { // <-- Check for ANY user (full or anonymous)

                // --- ADD THESE LINES ---
                // Anonymous users won't have userData, so we check for it
                const isFullUser = !!userData; // True if userData is not null
                const isAnonymous = user.isAnonymous;
                // --- END OF ADDED LINES ---

                // Redirect full users from index, but let anonymous users browse
                if (isFullUser && (window.location.pathname === '/' || window.location.pathname.endsWith('index.html'))) {
                ...
                // Use optional chaining 'userData?.' in case it's null
            ...
            if (userActions) {
                ...
                // ALWAYS show the cart button if a user (even anonymous) exists
                ...
                let userSpecificButtonsHTML = '';

                if (isFullUser) {
                    // Render full profile/notification buttons for real users
                    userSpecificButtonsHTML = `
                        <div class="relative">
                    ...
                    // Render "Login" & "Register" buttons for anonymous guest users
                     userSpecificButtonsHTML = `
                ...
                // Combine the buttons
                userActions.innerHTML = cartButtonHTML + userSpecificButtonsHTML;

                DarkMode.init(); // Initialize dark mode button
                setupCurrencySelector(); // Initialize currency selector

                // Add listeners for the buttons that were just added
                if (isFullUser) {
                ...
                // Add cart button event listener (this is now outside the "isFullUser" check)
