...
// Function to refresh prices for all cards in collection
export async function refreshAllPrices() {
    if (!state.currentUser) {
        throw new Error('User must be logged in to refresh prices');
    }

    console.log('[Collection] Starting price refresh for all cards...');
    const cards = state.fullCollection;
    let updatedCount = 0;
    
    for (const card of cards) {
        try {
            // Search for the card again to get fresh price data
            const searchResults = await API.searchCards(card.name, card.game || 'pokemon');
            
            // Find the matching card by api_id
            const updatedCard = searchResults.find(result => result.api_id === card.api_id);
            
            if (updatedCard && updatedCard.prices && Object.keys(updatedCard.prices).length > 0) {
                // Update the card with fresh price data
                const updates = {
                    prices: updatedCard.prices,
                    priceLastUpdated: new Date().toISOString()
                };
                
                await API.updateCardInCollection(state.currentUser.uid, card.id, updates);
                
                // Update local state
                const cardIndex = state.fullCollection.findIndex(c => c.id === card.id);
                if (cardIndex !== -1) {
                    state.fullCollection[cardIndex] = { ...card, ...updates };
                }
                
                updatedCount++;
                console.log(`[Collection] Updated prices for ${card.name}: ${JSON.stringify(updatedCard.prices)}`);
            }
        } catch (error) {
            console.error(`[Collection] Failed to refresh price for ${card.name}:`, error);
        }
    ...
    console.log(`[Collection] Price refresh complete. Updated ${updatedCount} cards.`);
