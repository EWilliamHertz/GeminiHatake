/**
 * HatakeSocial - Real-Time Notification System (v2 - Notification Center)
 */
    ...
    if (notificationCenterList) {
        ...
        notificationsRef.get().then(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                const notifEl = document.createElement('a');
                notifEl.href = notif.link || '#';
                notifEl.className = `block p-4 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700`;
                notifEl.innerHTML = `
                    <p class="text-gray-800 dark:text-gray-200">${notif.message}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">${new Date(notif.timestamp.toDate()).toLocaleString()}</p>
                `;
                notificationCenterList.appendChild(notifEl);
            });
        });
    }
});
