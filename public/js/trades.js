        ...
        case 'accepted':
            ...
            if (moneyInvolved) {
                if (isPayer) {
                    ...
                    return `<button data-id="${tradeId}" data-action="pay" class="trade-action-btn px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Pay ${currencySymbol}${convertedAmount.toFixed(2)} via Bank Transfer</button>`;
                } else {
...
window.handleTradeAction = async function(action, tradeId, db) {
    ...
    if (action === 'accepted') {
        ...
        showTradeToast("Loading payment details...", 'info');
        ...
        try {
            ...
            const paymentDetails = sellerData.paymentDetails;
            ...
            try {
                ...
                // Get currency symbol
                const currencySymbols = { USD: '$', EUR: '€', SEK: 'kr', GBP: '£' };
                currencySymbol = currencySymbols[userCurrency] || '$';
            } catch (error) {
                console.warn('Currency conversion failed, using USD amount:', error);
            }
            
            // Show payment modal with seller's payment details
            showPaymentModal(paymentDetails, convertedAmount, currencySymbol, tradeId);
            
        } catch (error) {
            console.error('Payment details error:', error);
            showTradeToast("Could not load payment details. Please contact the seller directly.", 'error');
        }
...
// Function to show payment modal with seller's payment details
function showPaymentModal(paymentDetails, amount, currencySymbol, tradeId) {
    ...
    if (!paymentDetails) {
        paymentHtml = `
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                <div class="flex">
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-red-800 dark:text-red-200">No Payment Details Available</h4>
                        <p class="text-sm text-red-700 dark:text-red-300 mt-1">
                            The seller has not configured their payment details yet. Please contact them directly to arrange payment.
                        </p>
                    </div>
                </div>
            </div>
        ...
        const hasSwedishPayment = paymentDetails.clearingKontonummer || paymentDetails.swishNummer;
        const hasInternationalPayment = paymentDetails.swiftIban;
        
        if (hasSwedishPayment) {
            paymentHtml += `
                <div class="mb-4">
                    <h4 class="text-sm font-semibold text-gray-800 dark:text-white mb-2">
                        <i class="fas fa-flag mr-2 text-blue-500"></i>Swedish Payment Methods
                    </h4>
            `;
            
            if (paymentDetails.clearingKontonummer) {
                paymentHtml += `
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-md mb-2">
                        <label class="text-xs font-medium text-gray-600 dark:text-gray-300">Clearing/Kontonummer:</label>
                        <p class="text-sm font-mono text-gray-900 dark:text-white">${paymentDetails.clearingKontonummer}</p>
                    </div>
                `;
            }
            
            if (paymentDetails.swishNummer) {
                paymentHtml += `
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-md mb-2">
                        <label class="text-xs font-medium text-gray-600 dark:text-gray-300">Swish Number:</label>
                        <p class="text-sm font-mono text-gray-900 dark:text-white">${paymentDetails.swishNummer}</p>
                    </div>
                `;
            }
        ...
        if (hasInternationalPayment) {
            paymentHtml += `
                <div class="mb-4">
                    <h4 class="text-sm font-semibold text-gray-800 dark:text-white mb-2">
                        <i class="fas fa-globe mr-2 text-green-500"></i>International Payment
                    </h4>
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-md">
                        <label class="text-xs font-medium text-gray-600 dark:text-gray-300">SWIFT/IBAN:</label>
                        <p class="text-sm font-mono text-gray-900 dark:text-white">${paymentDetails.swiftIban}</p>
                    </div>
                </div>
            `;
        }
        ...
        paymentHtml += `
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-4">
                <div class="flex justify-between items-center">
                    <span class="text-sm font-medium text-blue-800 dark:text-blue-200">Amount to Pay:</span>
                    <span class="text-lg font-bold text-blue-900 dark:text-blue-100">${currencySymbol}${amount.toFixed(2)}</span>
                </div>
            </div>
        `;
        ...
        if (paymentDetails.instructions) {
            paymentHtml += `
                <div class="mb-4">
                    <h4 class="text-sm font-semibold text-gray-800 dark:text-white mb-2">
                        <i class="fas fa-info-circle mr-2 text-blue-500"></i>Additional Instructions
                    </h4>
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-md">
                        <p class="text-sm text-gray-700 dark:text-gray-300">${paymentDetails.instructions}</p>
                    </div>
                </div>
            `;
        }
        
        if (!hasSwedishPayment && !hasInternationalPayment) {
            paymentHtml = `
                <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                    <div class="flex">
                        <i class="fas fa-exclamation-triangle text-yellow-600 dark:text-yellow-400 mr-3 mt-1"></i>
                        <div>
                            <h4 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">Incomplete Payment Details</h4>
                            <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                                The seller has not provided complete payment information. Please contact them directly to arrange payment.
                            </p>
                        </div>
                    </div>
                </div>
    ...
    const confirmBtn = document.getElementById('confirm-payment-sent');
    ...
    confirmBtn.onclick = async () => {
        try {
            // Update trade status to indicate payment has been sent
            const db = firebase.firestore();
            await db.collection('trades').doc(tradeId).update({
                status: 'payment_sent',
                paymentSentAt: firebase.firestore.FieldValue.serverTimestamp()
            });
            
            showTradeToast("Payment confirmation sent! The seller will ship once payment is received.", 'success');
            closeModal();
            
            // Refresh the trade display
            setTimeout(() => {
            ...
            showTradeToast("Could not confirm payment. Please try again.", 'error');
        }
    };
}
