...
def update_navigation_in_html(file_path):
    ...
    try:
        ...
        # Find any link going to articles.html that has a span with the text "Articles"
        all_links = soup.find_all('a', href=lambda href: href and 'articles.html' in href)
        sidebar_article_link = None
        for link in all_links:
            span = link.find('span')
            # Check for a span with the exact text "Articles"
            if span and span.get_text(strip=True) == 'Articles':
                # also check that the link is not for tcg or blog already
                if "?type=tcg" not in link.get('href', '') and "?type=blog" not in link.get('href', ''):
                    sidebar_article_link = link
                    break  # Found the link to modify
        
        if sidebar_article_link:
        ...
        index_header_blog_link = soup.find('a', href=lambda href: href and 'articles.html' in href, string=lambda t: t and t.strip() == "Blog")
